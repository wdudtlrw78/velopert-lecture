...
- dist 파일 상관없이 실행이된다.
- 결과물이 파일물로 쓰이지 않는다.
  dev-server는 `메모리`상에 존재하기 때문에 직접 파일을 쓰고 지우는 것 보다 더 빠르다.
- 파일 프로토콜이 아닌 환경으로 콘텐츠가 제공된다. (localhost와 같은 서버 제공하여 접근하기 쉽다.)
- HTTP URL로 접근하기 때문에 실제로 웹서비스에서 어떻게 이용해야할지에 대한 환경이 제공된다.
- 파일 변화를 감지했을 때 다시 build를 실행하는 `watch` 옵션이 적용되어있다.
- 실시간 리로딩 환경을 제공해준다. (편의성 향상)
- `historyApiFallback` : 특정 라우팅일 때 특정 지점으로 Fallback처리가 가능하다.
  - `historyApiFallback`이 `true` 경우 라우팅이 지원되지 않는 경로로 이동했을 때 기본적으로 index.html 파일로 이동시켜준다.
  - 보통 싱글페이지 어플리케이션과 같이 HTML5 history api 하는경우 설정한다.
  - `true` 말고 객체의 값을 할당할 수 있다.
  - 특정 경로 (라우팅 설정)를 특정 지점으로 이동할 수 있다 . `rewrites`
- `open`: true 는 기본 브라우저로 설정되어있는 브라우저 기반으로 새탭이 열리면서 웹어플리케이션이 실행된다.
- `overlay`: true는 에러 메세지가 발생했을 때 콘솔창이나 터미널 로그 값으로 확인하는 것이 아니라 에러 메세지 자체가 <b>브라우저 화면</b>에 나타난다.
  - create-react-app과 Cli 환경에서 overlay가 기본적으로 적용되어 있다.
- `port`: 임의의 port 값으로 수정가능한 역할을 한다.
